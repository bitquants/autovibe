...
import { Facebook, Twitter, Instagram, Youtube, Copy, Check, Edit2, Save, ImageIcon, Calendar } from 'lucide-react';
...
export function GeneratedPosts({ posts }: GeneratedPostsProps) {
  ...
  const [schedulingPost, setSchedulingPost] = useState<{ content: string; platform: Platform } | null>(null);
  ...
  const handleSchedule = async (post: { content: string; platform: Platform }) => {
    setSchedulingPost(post);
  };

  const handleScheduleSubmit = async (scheduledPost: Partial<{ platform: Platform; content: string; hashtags: string[]; scheduled_for: string }>) => {
    ...
    <>
            ...
            >
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center gap-1.5">
                  <button
                    onClick={() => handleSchedule({ content, platform: post.platform })}
                    className="p-1.5 rounded-md text-[#71717a] hover:text-[#a78bfa] hover:bg-[#a78bfa]/10 transition-colors"
                    title="Schedule post"
                  >
      ...
      {schedulingPost && (
        <ScheduleModal
          ...
          initialContent={schedulingPost.content}
          initialPlatform={schedulingPost.platform}
        />
      )}
    </>
  );
